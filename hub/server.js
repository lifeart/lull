...
import { buildAllowedOrigins, makeVerifyClient, resolveGroup, originAllowed, sameHost } from './auth.js';
import { UploadsManager, MAX_UPLOAD_BYTES } from './uploads.js';
...
// Multi-tenant (TOFU) mode: any distinct token becomes its own isolated family "group"; the
// groupId is a hash of the token (hub/auth.js), so no server-side registry is needed. Off by
// default → the hub is the single-group legacy system, 100% backward compatible.
const MULTIGROUP = process.env.MP_MULTIGROUP === '1';
const ALLOWED_ORIGINS = buildAllowedOrigins({ port: PORT, domain: process.env.MP_DOMAIN, extra: process.env.MP_ORIGIN });
// A tokenless client is admitted (to the shared DEFAULT_GROUP) only on loopback or with an explicit
// MP_ALLOW_OPEN opt-in. In multi-group mode a token IS required on a real network, and each token
// is its own isolated group — so a network reachable only with per-family tokens is safe.
const IS_LOOPBACK = HOST === '127.0.0.1' || HOST === 'localhost' || HOST === '::1';
const ALLOW_TOKENLESS = IS_LOOPBACK || process.env.MP_ALLOW_OPEN === '1';
const groupOpts = { multiGroup: MULTIGROUP, token: MP_TOKEN, allowTokenless: ALLOW_TOKENLESS };
const verifyClient = makeVerifyClient({ allowed: ALLOWED_ORIGINS, ...groupOpts });
...
const uploadsMgr = new UploadsManager(UPLOADS_DIR);

// Fail closed on a real network interface with no shared secret: an OPEN control channel there
// lets anyone on the LAN/tailnet start/stop/max-volume the speakers. Multi-group mode is exempt —
// there a token is REQUIRED on the network (verifyClient rejects tokenless clients unless
// MP_ALLOW_OPEN), and each token is its own isolated group, so no single shared secret is needed.
// Opt back into OPEN with MP_ALLOW_OPEN=1 (kept for the localhost dev box, which is trusted). (finding #14)
if (!MULTIGROUP && !MP_TOKEN && !IS_LOOPBACK && process.env.MP_ALLOW_OPEN !== '1') {
  console.error('[hub] REFUSING TO START: no MP_TOKEN set while bound to a non-loopback host ' +
    `(${HOST}). Set MP_TOKEN (e.g. \`openssl rand -hex 24\`), enable multi-group mode ` +
    '(MP_MULTIGROUP=1, per-family tokens), or to intentionally run OPEN on a trusted LAN set MP_ALLOW_OPEN=1.');
  process.exit(1);
}
...
// The sound library = baked noise loops (from the player manifest) + THIS GROUP's user uploads.
// Baked/ambient loops are shared read-only assets; uploads, favorites, and display order are all
// per-group so one family never sees another's tracks. The player resolves a soundscape id → url.
function libraryJson(groupId) {
  const uploads = uploadsMgr.for(groupId);
  const out = [];
  try {
...
// Gate every state-mutating /api route: browser Origin allowlist (CSRF) THEN resolve the caller's
// group from its token. Returns the groupId (always a non-empty string on success) or null after
// having already written the 403/401 — callers do `const g = apiGroup(req,res); if (!g) return;`.
function apiGroup(req, res) {
  if (!originOk(req)) { denyOrigin(res); return null; }
  const g = resolveGroup(req.url, groupOpts);
  if (!g.ok) { res.writeHead(g.code || 401).end(g.reason || 'unauthorized'); return null; }
  return g.groupId;
}

async function handleOrder(req, res) {
  const groupId = apiGroup(req, res); if (!groupId) return;
  const ids = (new URL(req.url, 'http://x').searchParams.get('ids') || '').split(',').filter(Boolean);
  await uploadsMgr.for(groupId).setOrder(ids);
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ ok: true }));
  hub.broadcastLibrary(groupId);
}
...
async function handleFav(req, res) {
  ...
  await uploadsMgr.for(groupId).setFav(id, q.get('on') !== '0');
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ ok: true }));
  hub.broadcastLibrary(groupId);
}
...
async function handleRename(req, res) {
  const groupId = apiGroup(req, res); if (!groupId) return;
  const q = new URL(req.url, 'http://x').searchParams;
  const item = await uploadsMgr.for(groupId).rename(q.get('id') || '', (q.get('name') || '').slice(0, 60));
...
async function handleDelete(req, res) {
  ...
  const ok = await uploadsMgr.for(groupId).remove(id);
...
async function handleForget(req, res) {
  const groupId = apiGroup(req, res); if (!groupId) return;
  const id = new URL(req.url, 'http://x').searchParams.get('id') || '';
  const ok = hub.forgetDevice(groupId, id); // a family can only forget its OWN devices
  if (!ok) { res.writeHead(404).end('not found'); return; }
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ ok: true }));
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (p === '/api/library') {
      // Read-only, so not Origin-gated — but the token still selects WHICH group's library to
      // return (baked/ambient are shared; uploads/favorites/order are per-group). A tokenless
      // request that can't resolve a group falls back to DEFAULT_GROUP (baked noises only).
      const g = resolveGroup(req.url, groupOpts);
      const groupId = g.ok ? g.groupId : DEFAULT_GROUP;
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' });
      res.end(JSON.stringify(libraryJson(groupId)));
      return;
    }
...
// The request carries the group verifyClient resolved from the token (req._groupId).
...
async function boot() {
  ...
  if (evicted.length) {
    const labels = evicted.map((e) => (e.groupId === DEFAULT_GROUP ? e.deviceId : `${e.groupId}/${e.deviceId}`));
    console.log(`[hub] evicted ${evicted.length} device(s) unseen >${DEVICE_TTL_DAYS}d: ${labels.join(', ')}`);
  }
  hub.startHeartbeat();
  server.listen(PORT, HOST, () => {
    ...
    console.log(`[hub]   mode       : ${MULTIGROUP ? 'MULTI-GROUP (per-token families, TOFU)' : 'single-group'}`);
    console.log(`[hub]   auth       : ${MULTIGROUP
      ? `per-family token${ALLOW_TOKENLESS ? ' (tokenless → shared default group allowed)' : ' required on the network'}`
      : (MP_TOKEN ? 'token required (MP_TOKEN set)' : 'OPEN (MP_ALLOW_OPEN / loopback)')}`);
    if (!MULTIGROUP && !MP_TOKEN && !IS_LOOPBACK) {
...
async function shutdown() {
  ...
  hardExit.unref();
  try { await Promise.all([store.flush(), uploadsMgr.flush()]); } catch (err) { console.error('[hub] flush on shutdown failed:', err.message); }
...
export { server, hub, store, uploadsMgr as uploads };
